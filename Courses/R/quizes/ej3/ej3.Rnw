...
\bigskip
Entrega tu código para resolver los siguientes ejercicios vía la página de cursos. Tu código debe ser portable y tiene que tener un nombre como lcollado.R
\bigskip
...
\begin{enumerate}
  ...
  \begin{enumerate}
	...
	\item Añadenle la línea de regresión lineal simple.
	\item Haz una predicción del valor de un diamante de un tercio de carat con esta regresión.
  \end{enumerate}
  \bigskip

  \item Usen el set de datos \texttt{cancer} del paquete \texttt{UsingR}. 
  \begin{enumerate}
    \item Haz una gráfica con los boxplots de las variables \texttt{stomach}, \texttt{bronchus}, \texttt{colon}, \texttt{ovary} y \texttt{breast}.
	\item Cual tiene la cola más larga?
	\item Cual es la más compacta?
  ...
  \begin{enumerate}
    \item Explora la relación entre el peso y la edad usando \texttt{lattice}.
    \item Particiona a las edades en intervalos de 0-3, 3-6, 6-9 y 9-12. Se mantiene la misma relación en los grupos?
  \end{enumerate}
  \bigskip
  ...
  \begin{enumerate}
	...
	\item Encuentra el resumen de estadísticas para cada grupo racial.
  \end{enumerate}
  \bigskip
